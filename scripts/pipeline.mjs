...
//   --from-content <f> land a draft from a pre-generated content JSON file
//                     (same shape as DRAFT_SCHEMA), NO API call. Lets an
//                     orchestrating agent supply the AI-generated content and
//                     still round-trip it through the real authoring writer.
//
// Requires ANTHROPIC_API_KEY in the environment for real SDK runs. Model spend is
...
function parseArgs(argv) {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    else if (a === '--from-content') opts.fromContent = argv[++i];
...
if (opts.fromContent) {
  // No API call: land a pre-generated content object (the same shape the model
  // returns) through the real authoring writer. This is the agent-supplied path
  // — an orchestrating agent generates the draft and round-trips it here, so the
  // brief -> publishable draft -> workflow loop works with zero external spend.
  let draft;
  try {
    draft = JSON.parse(readFileSync(opts.fromContent, 'utf8'));
  } catch (e) {
    die(`cannot read/parse content file ${opts.fromContent}: ${e.message}`);
  }
  for (const k of ['title', 'description', 'category', 'tags', 'body']) {
    if (draft[k] == null) die(`content file missing required field "${k}"`);
  }
  ...
  console.log(`✓ agent-supplied draft landed in the workflow: ${path}`);
  console.log(`  title: ${draft.title}`);
  console.log(`  draft length ~${words} words`);
  console.log(`  next:  node scripts/content.mjs status ${slug} review`);
  console.log('\n  unit economics: no external API spend (content generated by the orchestrating agent).');
  process.exit(0);
}
